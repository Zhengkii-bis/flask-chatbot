from flask import Flask, render_template, request import textstat import requests from spellchecker import SpellChecker import re from textblob import TextBlob
...
def restate_essay(text): blob = TextBlob(text) corrected_text = blob.correct() return str(corrected_text)

@app.route('/', methods=['GET', 'POST']) def index(): feedback = "" restated_essay = "" if request.method == 'POST': essay = request.form['essay'] word_count = len(essay.split()) spelling_mistakes = list(spell.unknown(essay.split())) readability = textstat.flesch_reading_ease(essay) grammar_feedback = check_grammar(essay) organization_feedback = analyze_organization(essay) restated_essay = restate_essay(essay)
